<script lang="ts">
	...
	const data = tsv.split('\n').map((str) => {
		...
		return { date, avg: avg === -99.99 ? undefined : avg, trend };
	});

	const points = data
		.filter((d) => typeof d.avg !== 'undefined')
		.map(({ date, avg, trend }) => ({ date, avg: avg!, trend }));

	let minx = points[0].date;
	let maxx = points[points.length - 1].date;
	let miny = +Infinity;
	let maxy = -Infinity;
	let highest: (typeof points)[number];
...
<div class="chart">
	<Pancake.Chart x1={minx} x2={maxx} y1={miny} y2={maxy}>
		...
		<Pancake.Quadtree data={points} x={(d) => d.date} y={(d) => d.avg} let:closest>
			{#if closest}
				<Pancake.Point x={closest.date} y={closest.avg}>
					<div class="focus"></div>
