<script lang="ts">
	...
	import { default_x, default_y } from '$lib/utils/accessors.js';
	...
	function get_populations(year: number, sex: number) {
	...
	const handle_pointerdown = (e: PointerEvent) => {
		...
		const handle_pointermove = (e: PointerEvent) => {
			if (!e.isPrimary) return;
		...
		const handle_pointerup = (e: PointerEvent) => {
			if (!e.isPrimary) return;
...
<div class="chart {size}" bind:this={el}>
	<div class="background">
		<Pancake.Chart x1={$x1 - 2.5} x2={$x2 + 2.5} y1={0} y2={max} clip>
			<!-- men -->
			<Pancake.Columns data={$m} x={default_x} y={default_y} width={5}>
				<div class="column m"></div>
			</Pancake.Columns>

			<!-- women -->
			<Pancake.Columns data={$f} x={default_x} y={default_y} width={5}>
